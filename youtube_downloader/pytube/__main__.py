...
class YouTube(object):
    ...
    def init(self):
        ...
        self.vid_info = {k: v for k, v in parse_qsl(self.vid_info)}         ## parse_sql returns (key,val) tuple list 
        if self.age_restricted:                                             ## by using dict comprehension it turns into dict
            self.player_config_args = self.vid_info
        else:
            self.player_config_args = extract.get_ytplayer_config(          ## here executed
                self.watch_html,
            )['args']
        ...
        for fmt in stream_maps:
            if not self.age_restricted and fmt in self.vid_info:        #vid_info is invalid in case of "https://www.youtube.com/watch?v=AMv_uaqPNt4"
                mixins.apply_descrambler(self.vid_info, fmt)            #so pass this line
            
            ## before mine
            # with open("player_config_args0.txt","w", encoding="utf8") as file:
            #     for key, val in self.player_config_args.items():
            #         file.write('{0}:{1}'.format(key, val))
            
            mixins.apply_descrambler(self.player_config_args, fmt)      
            # with open("player_config_args1.txt","w", encoding="utf8") as file:
            #     for key, val in self.player_config_args.items():
            #         file.write('{0}:{1}'.format(key, val))    
            ## after
            ## TODO: see if adaptive_fmts is also applied
            try:
    ...
    def prefetch(self):
        ...
        operations don't need to make calls outside of the interpreter
    ...
    @property
    def author(self):
        """Get the author or channel name
