...
class Stream(object):
    ...
    def __init__(self, stream, player_config_args, monostate):
        ...
        self.resolution = None   # resolution (e.g.: 480p, 720p, 1080p) ## To Fix res -> resolution
    ...
    def filesize_mb(self):
        """File size of the media stream in Gbytes.

        :rtype: int
        :returns:
            Filesize (in Gbytes) of the stream.
        """    
        byte = self.filesize
        div = 1024 * 1024
    ...
    def download(self, output_path=None, filename=None, filename_prefix=None):
        ...
        with open(fp, 'wb') as fh:
            for chunk in request.get(self.url, streaming=True, chunk_size = 128 * 1024):
